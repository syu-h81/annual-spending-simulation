    ...
    computed: {
        ...
        residentTax: function() {
            if(this.price < 1950000) {
                return Math.trunc((this.price * 0.05) / 10).toLocaleString();
            }
            if(this.price >= 1950000 && this.price < 3300000) {
                return Math.trunc((this.price * 0.1 - 97500) / 10).toLocaleString();
            }
            if(this.price >= 3300000 && this.price < 6950000) {
                return Math.trunc((this.price * 0.2 - 427500) / 10).toLocaleString();
            }
            if(this.price >= 6950000 && this.price < 9000000) {
                return Math.trunc((this.price * 0.23 - 636000) / 10).toLocaleString();
            }
            if(this.price >= 9000000 && this.price < 18000000) {
                return Math.trunc((this.price * 0.33 - 1536000) / 10).toLocaleString();
            }
            if(this.price >= 18000000 && this.price < 40000000) {
                return Math.trunc((this.price * 0.4 - 2796000) / 10).toLocaleString();
            }
            if(this.price >= 40000000) {
                return Math.trunc((this.price * 0.45 - 4796000) / 10).toLocaleString();
            }
        },
        /////国民年金の算出方法/////
